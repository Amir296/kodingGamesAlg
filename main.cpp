...
bool
MakeAMove (T_Zone& zzFrom,const T_Zone& zzDest, T_Move& tm)
{
       if(zzFrom.myPods < 1)
        return false;
...
#ifdef OLD 
        if(zzDest.myPods < zzFrom.myPods  && (zzFrom.myPods > 1 || zzFrom.ownerId != myId))// || (0 == zzDest.myPods && zzFrom.myPods > 0)  )   
        {
           tm.zoneTo = zzDest.zId;
           tm.numOfPods = zzFrom.myPods > 3 ? zzFrom.myPods/3 : 1;
           zzFrom.myPods -= tm.numOfPods;
           return true; 
        }
        
         if(zzFrom.myPods > 0 && zzFrom.ownerId != myId)   
        {
           tm.zoneTo = zzDest.zId;
           tm.numOfPods = zzFrom.myPods > 3 ? zzFrom.myPods/3 : 1;
           zzFrom.myPods -= tm.numOfPods;
...
//do not leave platinum
        bool isCond = false;
        if(zzFrom.platinum == 0 || zzFrom.myPods > 1 && zzFrom.platinum > 0)
        {
            
            //search for platinum
            if(zzDest.platinum > 0)
            {
                isCond = true;
                tm.numOfPods = 1;
            }
            
//dfs stile, run forward
        if(distFromEnemyHQ[zzDest.zId] < distFromEnemyHQ[zzFrom.zId]   )
            {
                tm.numOfPods = (zzFrom.myPods < 5) ? 1 :  zzFrom.myPods/2;
                return true; 
            }


        //  go_spread_around        
         if(distFromMyHQ[zzDest.zId] > distFromMyHQ[zzFrom.zId])
           {
                isCond = true;
                tm.numOfPods = 1;
           }
           
           //  take enemy.. or nautral zone
         if(zzDest.ownerId != myId)
           {
                isCond = true;
                tm.numOfPods = 1;
           }
